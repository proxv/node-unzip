  ...
  this._pullStream.pull(26, function (err, data) {
    ...
    return self._pullStream.pull(vars.fileNameLength, function (err, fileName) {
      ...
      self._pullStream.pull(vars.extraFieldLength, function (err, extraField) {
        ...
        if (vars.compressionMethod === 0) {
          self._readUncompressedFileInChunks(entry, vars.compressedSize);
        } else {
...
Parse.prototype._readUncompressedFileInChunks = function(entry, fileSize, chunkNum) {
  var self = this;
  chunkNum = chunkNum || 0;
  var chunkSize = self._opts.chunkSize || fileSize;
  var numChunks = Math.ceil(fileSize / chunkSize);
  var bytesToRead = Math.min(chunkSize, fileSize - chunkSize * chunkNum);
  var hasEntryListener = self._hasEntryListener;
  if (chunkNum < numChunks) {
    self._pullStream.pull(bytesToRead, function (err, compressedData) {
      if (err) {
        return self.emit('error', err);
      }
      ...
      self._readUncompressedFileInChunks(entry, fileSize, chunkNum + 1);
    });
  } else {
    if (hasEntryListener) {
      entry.end();
